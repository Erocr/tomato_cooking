...
class View:
    ...
    def draw_grid(self, grid, t):
        pg.draw.rect(self.display, (200, 200, 200), pg.Rect(0, 0,
                                                            *self.grid2screen(Vec(len(grid.grid), len(grid.grid[0]))).get()))
        ...
        for y in range(len(grid.grid)):
            for x in range(len(grid.grid[y])):
                for tomato in grid.tomatoes[y][x]:
                    self.draw_tile(self.grid2screen(tomato.lerped_position(t)+Vec(0.25, 0.25)), "tomates", tomato.type)
